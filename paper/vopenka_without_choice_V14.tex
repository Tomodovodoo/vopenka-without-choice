...
\begin{proof}
Relativizing each quantifier of $\varphi$ to $a$, transitivity and elementarity of $j$ give, for every formula $\varphi$ and every finite tuple $\bar x$ from $a$,
\[
...
\]
 in the set-sized language with membership, one distinguished constant, and constants for $V_{\bar\gamma+1}$.  The structures $\mathcal B_\lambda$ form a $\Pi_1$-definable proper class: the hierarchy equation is $\Pi_1$, ordinal addition is $\Delta_1$, and the interpretation tables are bounded in the code.  Applying $\VP(\Pi_1)$ to this class gives distinct ordinals $\lambda,\mu$ and an elementary embedding $j:\mathcal B_\lambda\longrightarrow\mathcal B_\mu$.
...
\begin{theorem}[Woodin, restricted formulation]\label{thm:Woodin-forcing}

Assume that $\AC$ fails, let $\mu$ be least such that $\DC_\mu$ fails, and let $\Lambda>\mu$ be supercompact in Woodin's choiceless sense.  There is a weakly homogeneous forcing $\mathbb Q_\Lambda$, uniformly first-order definable over $V_\Lambda$, such that whenever $G\subseteq\mathbb Q_\Lambda$ is generic:
...
Let $\mathcal D_{\mathrm W}$ be a fixed finite list of formulas defining this presentation, its complete codes $a_\eta$, and marked stages.
...
\]
The reflected instances of $\Xi_N$ give $\bar\theta\in C^{(r)}$, show that $\bar\theta$ is supercompact in Woodin's sense, and identify $\bar x=a_{\bar\theta}$.
...
\begin{proof}
The model in \eqref{eq:full-Solovay} is the full Solovay model in the precise convention of \cite[Definition~4 and the following paragraph]{SakaiTanno}.  Since a measurable cardinal is inaccessible in $\ZFC$, Solovay's argument gives
...
Consequently, by \cite[Proposition~9.4]{KaragilaSchilhan}, using same-extension comparison generics as in \cite[Karagila--Schilhan supplement]{RepairNotes}, the symmetric extension arising from

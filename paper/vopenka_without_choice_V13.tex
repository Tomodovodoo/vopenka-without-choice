...
\begin{proof}
{\color{auditgreen}
For each formula $\varphi$, relativize every quantifier to the parameter $a$.  Transitivity of $M$ makes the resulting formula compute truth in $(a,\in)$, and transitivity of $N$ does the same for $j(a)$.  Elementarity of $j$ therefore gives, for every formula $\varphi$ and every finite tuple $\bar x$ from $a$,
}
\[
...
{\color{auditgreen}
Assume $\ZF+\VP(\Pi_1)$.  Above every ordinal $\gamma$ there is a limit ordinal $\kappa$ such that no map from any $a\in V_\kappa$ is cofinal in $\kappa$.  Consequently $V_\kappa\models\ZF$.  Under $\AC$, such a $\kappa$ may be chosen measurable.
}
\end{lemma}
...
\]
{\color{auditgreen}
 in the set-sized language with membership, one distinguished constant, and constants for $V_{\bar\gamma+1}$.  The structures $\mathcal B_\lambda$ form a $\Pi_1$-definable proper class, using $V_{\bar\gamma+1}$ as a parameter: the hierarchy equation for the domain has a $\Pi_1$ definition, ordinal addition is $\Delta_1$, and the distinguished ordinal and interpretation tables are extracted by bounded quantifiers from the structure code.  Applying $\VP(\Pi_1)$ to this class gives distinct ordinals $\lambda,\mu$ and an elementary embedding $j:\mathcal B_\lambda\longrightarrow\mathcal B_\mu$.
}
...
\begin{proof}
{\color{auditgreen}
For each fixed standard formula of positive L\'evy level $k\geq1$, symmetric forcing has a definition of the same polarity and level, uniformly in the ground model and the symmetric system.  The bounded base has a $\Delta_1$ definition.  To obtain it, use set certificates for the hereditary name closure, the action and stabilizer tables, and the atomic forcing recursion.  Existence and uniqueness of these recursion tables give the two $\Sigma_1$ and $\Pi_1$ presentations; bounded quantifiers range over subnames and conditions.  At an unbounded existential step the clause is that below every condition there is a stronger condition and an HS name forcing the matrix.  The condition quantifiers are bounded by the set $\mathbb P$, and the name quantifier gives the required $\Sigma_k$ presentation.  Negation gives the dual $\Pi_k$ presentation.  Collection supplies the set certificates used in this induction; no selection of one name for each condition is required.
}
...
{\color{auditgreen}
Lemma~\ref{lem:ZF-ranks} applies because $\VP(\Pi_n)$ includes $\VP(\Pi_1)$.  It gives unboundedly many ordinals satisfying the $\Pi_1$ condition of Lemma~\ref{lem:ZF-rank-criterion}.  Write $B(W,\theta,b)$ for the bounded assertion that, for every $b<\eta<\theta$, either $\eta$ is not a limit above $\omega$, or $W$ contains a cofinal map $f:a\to\eta$ and a transitive rank-table certificate showing $\rank(a)<\eta$.  If $\theta$ is the least rank-criterion height above $b$, then $B(V_\theta,\theta,b)$ holds: for each $\eta<\theta$, the map and a rank table on $V_\eta$ belong to $V_\theta$, since $\theta$ is a limit.  Conversely, any witness to $B$ certifies failure of the criterion at every earlier candidate.  Apply the preceding least-witness coding observation to $\exists W\,B(W,\theta,b)$.  The resulting unique certificate $d$, together with the positive criterion at $\theta$, gives a $\Pi_1$ definition of $(\theta,d)$ from $b$.  The available witness $V_\theta$ has rank $\theta$, so the least witness rank is at most $\theta$ and the entire certificate $d$ belongs to $V_{\theta+\omega}$.  The satisfaction relation of $V_\theta$ also belongs to that rank.
}
...
We recall that a nonzero cardinal $\delta$ is rank-Berkeley if, for every $\zeta<\delta$ and every $\Theta>\delta$, there is a nontrivial elementary self-embedding $j:V_\Theta\to V_\Theta$ with $\zeta<\crit(j)<\delta$ and $j(\delta)=\delta$ \cite[Definition~4.4]{Mohammd}.
...
The next definitions record finite bounds on the L\'evy complexity needed for reflection.  Fix a G\"odel numbering of formulas and a primitive-recursive prenex-normal-form operation.  For a concrete complexity bound, expand defined predicates and terms using their fixed graph formulas, rename bound variables to avoid capture, replace bounded quantifiers by unbounded quantifiers with membership guards, and eliminate implication and biconditional.  If $q(e)$ counts all quantifier occurrences in the resulting formula, put $\ell(e)=q(e)+1$.  Prenex conversion has at most $q(e)$ alternating blocks; a vacuous leading block of either polarity bounds both the formula and its negation by $\ell(e)$.  This is a conservative primitive-recursive bound and uses no axioms of the rank structures.
...
For an ordinal $\xi$, let $\DC_\xi$ denote dependent choice for
histories of length $\xi$.  We use Woodin's collapse
\[
C_\nu^\kappa=\{p:\operatorname{dom}(p)\subseteq\nu\times\kappa,\
 |\operatorname{dom}(p)|<\nu,\ p(i,\zeta)\in V_{1+\zeta}\},
\]
whose conditions are partial functions, ordered by reverse inclusion.
Here $\nu$ is an infinite regular cardinal; ranks and the lower
index are interpreted in the universe preceding that collapse.

Use the following consistent version of the iteration in
\cite[pp.~323--324]{Woodin}.  Let $\mu$ be the least failure of dependent
choice, start with the trivial $\mathbb Q_0$ and $\kappa_0=\mu$,
and at each successor append $C_{\kappa_\alpha}^{\kappa_{\alpha+1}}$,
where $\kappa_{\alpha+1}$ is the least successful ground strongly
inaccessible cutoff.  At a limit take the direct limit if the
supremum of the earlier cutoffs is ground strongly inaccessible.
Otherwise first take the raw inverse limit and then append the
least successful restoration collapse.  Its lower index is the
raw extension's own Hartogs successor of that supremum.
A completed prefix includes this appended collapse.
A stage $\gamma$ is \emph{marked} when $\kappa_\gamma=\gamma$.

Fix the bounded sparse presentation with globally valid normalized
coordinate names.  Guard a locally valid name by its condition, with
the empty name elsewhere, and normalize by taking the union of all
globally equivalent names of the least possible rank.  This is a
specified representative, uses no choice of representatives, and
fixes the empty name.  Conditions omit empty coordinates; projections
are restrictions and inclusions add empty tails.  At direct cuts
supports are bounded, and inverse limits retain the support
requirements at all earlier direct cuts.

Let $a_\eta=\langle s_\eta,K_\eta\rangle$ be the actual complete code
through $\eta$, including the seed.  The six fields of $s_\eta$
are the carriers, orders, tops, projections, empty-tail inclusions,
and prefix-replacement maps; $K_\eta$ is the cutoff table.
The preceding convention defines the name pools from these data.
At a marked $\eta$, every condition in every earlier row is in
$V_\eta$, so finite pairing gives $a_\eta\in V_{\eta+\omega}$.

\begin{theorem}[Woodin's construction and a restricted lift]\label{thm:Woodin-forcing}
Assume that $\AC$ fails, let $\mu$ be least such that $\DC_\mu$ fails,
and let $\Lambda>\mu$ be supercompact in Woodin's choiceless sense.
The specified iteration is defined through $\Lambda$.
Its endpoint $\mathbb Q_\Lambda$ is weakly homogeneous and uniformly
$\Sigma_3$-definable over $V_\Lambda$.
If $G\subseteq\mathbb Q_\Lambda$ is generic, write $W=V[G]$ and let
$G_\alpha$ be its induced completed-prefix generic.
\begin{enumerate}
\item $W_\Lambda\models\ZFC$.
\item Every marked $\gamma\leq\Lambda$ is a direct stage, remains
strongly inaccessible in $W$, and satisfies
\[
V_\gamma[G_\gamma]=(V[G_\gamma])_\gamma=W_\gamma\models\ZFC.
\]
The iteration factors over its completed prefixes by the canonical
complete projections.  Every ground supercompact
$\gamma\leq\Lambda$ is marked.  In $V[G_\alpha]$, a later quotient
has a lower bound in its separative preorder for every directed
family enumerated by an ordinal below $\kappa_\alpha$.
It preserves $\DC_{<\kappa_\alpha}$ and adds no shorter sequences
of old sets.
If $t\in\mathbb Q_\Lambda$ has $\alpha$-prefix in $G_\alpha$,
there is a generic $G^*$ with
\[
t\in G^*,\qquad G_\alpha^*=G_\alpha,\qquad V[G^*]=W.
\]
\item Suppose $J:V_{\bar\gamma+\omega}\to V_{\gamma+\omega}$ is
elementary, $J(\bar\gamma)=\gamma<\Lambda$,
\[
\crit(J)=\bar\delta<\bar\gamma<J(\bar\delta)=\delta,
\qquad \kappa_\gamma=\gamma,\qquad J(a_{\bar\gamma})=a_\gamma.
\]
Then $\bar\gamma,\bar\delta,\delta$ are marked, and in $W_\Lambda$
there is an elementary embedding
\[
j:W_{\bar\gamma}\longrightarrow W_\gamma
\]
with $\crit(j)=\bar\delta$, $j(\bar\delta)=\delta$, and
$j(x)=J(x)$ for every ground $x\in V_{\bar\gamma}$.
\end{enumerate}
\end{theorem}

\begin{proof}
We give the construction and rank details needed for the restricted
lift.  The large-cardinal input is the small-embedding characterization
in \cite[Lemma~222]{Woodin}.  Its critical points, and the supercompact
cardinals themselves, are strongly inaccessible in the following
choiceless sense: every map $V_\xi\to\kappa$, $\xi<\kappa$, has
bounded range in $\kappa$.  A contrary map at a critical point would
be fixed on its domain and values while its image must be cofinal
above that critical point.  The same characterization, applied to
$\Sigma_2$ witnesses at sufficiently correct target ranks, gives
$V_\kappa\prec_{\Sigma_2}V$ for supercompact $\kappa$.
Strong inaccessibility gives $V_\kappa\models\ZF$ by bounding the
output ranks in Replacement.  A least failure of dependent choice
is regular: at a singular least failure, dependent choice builds
cofinally many coherent shorter branches, whose union is a full
branch.  Failure of Choice and failure of dependent choice are
$\Sigma_2$, so this least failure is below every supercompact.

We first record the strict restoration step, including its use
after a small prefix.  If $P,\leq_P,1_P\in V_\Delta$, $\Delta$ is
ground supercompact, and
\[
P\Vdash \nu<\Delta\text{ is infinite regular and }\DC_{<\nu},
\]
then, above any prescribed bound below $\Delta$, there is a ground
strongly inaccessible $\lambda<\Delta$ such that
\[
P*\dot C_\nu^\lambda\Vdash
 \operatorname{Reg}(\lambda)\land\DC_{<\lambda}.
\]
For the non-strict upper bound $\Delta$, use the proof of
\cite[Lemma~225]{Woodin} with a ground small embedding fixing
$P,\nu$ and its order.  It captures a name for the serial relation.
The source collapse is the restriction below the critical point.
Short histories in the smaller extension stay there by closure
and $\DC_{<\nu}$; the full collapse well-orders a bounded pool of
names for the source relation.  Choosing its least admissible
successor constructs a branch of length $\nu$ without
$\DC_\nu$.  Applying the lift pointwise gives the required branch
in the target.  The upper inaccessible stays regular: close a
small column subcollapse under least bounds for deciding a proposed
short cofinal map; strong inaccessibility bounds the possible
decisions.  All ordinals below the upper cutoff then have
cardinality at most $\nu$.  Ordinal induction, using coherent
cofinal blocks at limits, gives $\DC_{<\Delta}$.

To make the cutoff strict, take a further small embedding with
critical point above the prescribed bound and the prefix data.
Dependent choice is absolute between a small forcing extension
and its $\Sigma_1^*$-correct rank extension.  Here the extra
reflection is used as follows.  A failure has a certificate in a
transitive ZF set model closed under the required short functions.
Such closed models exist by ground supercompactness.  Their
closure survives the small forcing: for each condition and
argument, use the least-rank union of all names forced to give
the same value, and collect the resulting uniquely specified
function.  A supplied closed model, its satisfaction relation
and the bounded forcing certificate express the failure by a
closed-set assertion reflected by $\Sigma_1^*$ correctness.
Thus a genuine failure reflects to the rank extension, and
conversely all relevant short histories of its sets lie in that
rank.  Applying this equivalence to the lifted small embedding
reflects restoration at $\Delta$ to restoration at its critical
point.  Regularity of this smaller upper cutoff follows from the
same decision-bound argument.  This proves strict restoration
without assuming that $P$ preserves supercompactness.

For the quotient bound, fix a completed base $B=\mathbb Q_\alpha$,
a later stage, and a base name $\dot f$ for a directed family with
enumeration length $\nu<\kappa_\alpha$.  Choose $p$ forcing its type.
List all ground triples $(d,\xi,q)$ for which
\[
d\leq p,\ q\restr\alpha,\qquad
d\Vdash_B\dot f(\xi)=\check q .
\]
For each $(d,\xi)$ there is at most one $q$.  Construct a bound in
the ground by recursion, retaining the invariant that, whenever
$u$ strengthens its prefix through $i$ and
$r\leq u\restr\alpha,d$, replacement of that prefix by $r$ gives
$u[r]\leq q\restr i$.

At a collapse coordinate take the union of the selected tail names,
guarded by $d$, the constructed prefix bound and the selected
condition's prefix.  In any generic containing the constructed
prefix, the invariant puts every selected original prefix into
that generic.  The union therefore evaluates to the union of the
selected collapse graphs.  These graphs are compatible:
separative comparison projects through prefix replacement, and
a contrary evaluated tail extension would lift to a contrary
two-step condition.  A canonically enumerated union of fewer than
the regular lower-index number of domains is still small.
The ranks of the possible names are indexed by the low-rank set
$B\times\nu$, so ground strong inaccessibility bounds them below
the upper cutoff.  Normalization preserves the forced comparisons.
At each inaccessible direct cut the least support bounds are
indexed by the same set, so all possible supports have a common
bound below that cut.  At inverse cuts take the coherent ground
thread.  The invariant uses one and the same base strengthening
at every coordinate.  At the desired endpoint it says that every
extension of the constructed condition is compatible with every
member of the family in the quotient.  This is the required
separative lower bound.

These bounds also give the raw inverse-limit assertion used in
the construction.  Put $\lambda=\sup_{\xi<\beta}\kappa_\xi$ at a
non-inaccessible limit.  The earlier completed quotients preserve
their $\DC_{<\kappa_\xi}$ and regular cardinals, so the raw
extension satisfies $\DC_{<\lambda}$ and $\lambda$ remains a
cardinal.  Failure of ground strong inaccessibility supplies an
unbounded $V_\rho\to\lambda$.  An earlier collapse enumerates its
domain by an ordinal below $\lambda$, making $\lambda$ singular
in the raw extension.  Dependent choice on coherent cofinal
blocks gives $\DC_\lambda$, hence $\DC_{<\lambda^+}$ since a
least failure is regular.  It also makes $\lambda^+$ regular:
a short cofinal sequence and the choice of injections of its
entries into $\lambda$ would inject $\lambda^+$ into $\lambda$.
This proves the lower-index hypotheses before the completion
collapse is appended.

At successors strict restoration now supplies the least cutoff.
At limits below $\Lambda$, ground strong inaccessibility bounds
the earlier cutoffs and the raw forcing rank below $\Lambda$.
Use the direct rule at inaccessible suprema and the preceding
raw argument followed by strict restoration otherwise.
This simultaneous induction supplies every stage, its complete
projections, regular cutoff and dependent choice, with
$\alpha\leq\kappa_\alpha<\Lambda$ for $\alpha<\Lambda$.
The quotient-bound recursion applies equally at completed
successors and at the final direct row.  Repeating the induction
with any ground supercompact $\gamma\leq\Lambda$ as outer bound
gives $\sup_{\alpha<\gamma}\kappa_\alpha=\gamma$.
Thus each such $\gamma$ is marked.

Weak homogeneity has a specified construction.  For two collapse
conditions, swap the rows used by the first with the first equally
many unused rows outside both supports.  The moved condition is
compatible with the second.  At each coordinate transport this
operation by the earlier automorphism and normalize its name.
Inverse row permutations give its inverse.  Empty coordinates
stay empty, and common-extension supports lie in the union of
the two original supports, preserving all direct cuts.
Starting the recursion above a common completed prefix fixes
that prefix pointwise.  Given $t$ whose prefix lies in the
generic, conditions that some such ground automorphism sends
below $t$ are dense below that prefix.  Meeting this set gives
$G^*$ with $t\in G^*$, the same prefix generic and the same full
extension.  This also makes each raw inverse-limit successor
cardinal a constant ordinal, as required by the recursion.

For definability use bounded hierarchy, subname and forcing
certificates for these fixed presentation operations.
Their graphs have $\Delta_2$ definitions.  The assertion
$\DC_{<\kappa}$, and its set-forcing version at a supplied
presentation, have $\Pi_2$ definitions.  A least successful cutoff
is consequently described by its positive $\Pi_2$ test and the
$\Sigma_2$ failures at all smaller candidates.
A supplied entire prefix therefore has a $\Sigma_3$ definition.
At $V_\Lambda$, $C_2$ correctness identifies the positive and
negative tests with their ground meanings.  The bounded operations
and uniqueness of the recursion identify its actual prefixes.
A sparse endpoint condition belongs to some such prefix, giving
the uniform $\Sigma_3$ definition of the endpoint.

We prove the rank assertion for the actual stages.  Put
$M_i=V[G_i]$.  The induction invariant is
\[
M_i\models
 \forall\rho<i\ \exists\nu<\kappa_i\
 \exists f:\nu\twoheadrightarrow V_\rho.
\]
It is vacuous at zero.  Separative closure and $\DC_\nu$ decide a
name for a sequence of length $\nu<\kappa_i$ by a descending
history and a final bound.  Thus the tail adds no such sequence
of old sets.  An old short enumeration of $X$ then prevents new
subsets of $X$, by taking inverse images of characteristic
functions.  At a successor the invariant therefore preserves
all ranks through $i$, and the actual next collapse enumerates
them by $\kappa_i<\kappa_{i+1}$.  At a limit $\theta$, for
$\rho<\theta$ use the specified earlier stage $i=\rho+1$.
Its enumeration still enumerates the same rank after the
completed quotient, and its length is below
$\kappa_i<\kappa_\theta$.  This also proves the assertion at the
separately constructed terminal row $\Lambda$.

At a marked $\gamma$, these enumerations and regularity bound
every function from a smaller extension rank into $\gamma$.
They give Replacement in $(M_\gamma)_\gamma$ by bounding output
ranks, and Choice by choosing least enumeration indices.
The remaining ZF axioms follow from transitivity and finite rank
bounds.  The later quotient preserves all ranks through $\gamma$
and its regularity, so these assertions hold in $W$.
Every member of $W_\gamma$ has a name in $V_\gamma$:
place it in an earlier stabilized prefix and use the name pools
\[
B_0=\varnothing,\qquad B_{\xi+1}=\mathcal P(B_\xi\times P),
\qquad B_\lambda=\bigcup_{\xi<\lambda}B_\xi .
\]
Their evaluations cover the corresponding ranks by the
all-witness atomic-forcing name construction.  For each fixed
$\xi<\gamma$ their rank is below
$\zeta+\omega\cdot(\xi+1)<\gamma$ for a suitable $\zeta<\gamma$.
Conversely a name in $V_\gamma$ evaluates to a set of rank below
$\gamma$.  This proves (1) and the rank equalities in (2).

Finally assume (3), and write $e=\bar\delta$.
The captured cutoff table gives
$\kappa_{\bar\gamma}=\bar\gamma$.
For $i<e$, covariance of the actual cutoff tables gives
$J(\kappa_i)=\kappa_i$.  If $\kappa_i\geq e$, then
$\kappa_i\geq J(e)=\delta>\bar\gamma$, a contradiction.
Thus $\kappa_i<e$ for every $i<e$.
The critical-point argument makes $e$ strongly inaccessible;
$\sup_{i<e}\kappa_i=e$, so $e$ and its image $\delta$ are marked.
Also $\delta<\gamma$.
Every individual condition of $\mathbb Q_e$ belongs to $V_e$
and is fixed.  For $q\in G_{\bar\gamma}$,
\[
J(q)\restr\delta=J(q\restr e)=q\restr e\in G_\delta.
\]
Thus $D=J''G_{\bar\gamma}$ is directed in
$\mathbb Q_\gamma/G_\delta$ and belongs to $V[G_\delta]$.
The source generic has rank below $\bar\gamma+2<\delta$.
The proved enumeration invariant at $\delta$ gives it an
ordinal enumeration of length below $\delta$; applying $J$
pointwise gives one for $D$.
Take its separative quotient bound $t$ and use (2) at $\delta$.
Then $G^*$ has the same full extension, keeps
$G_{\bar\gamma}$ fixed, and satisfies
\[
J''G_{\bar\gamma}\subseteq G_\gamma^* .
\]
Indeed, a generic containing a separative lower bound of $q$
meets the dense set of conditions below $q$ or incompatible
with $q$, and the incompatible alternative is impossible.

For $\eta=\bar\gamma,\gamma$, let
$N_\eta$ be the set of $\mathbb Q_\eta$-names in $V_\eta$.
These subname-closed sets are mapped correctly by $J$.
The atomic forcing tables on $N_\eta$, followed by the
negation, conjunction and existential-density clauses with
witnesses in $N_\eta$, give a forcing table for the set of
their evaluations.  Finite assignments are finite functions
in $V_\eta$, so the whole table and its atomic certificates
belong to $V_{\eta+\omega}$.  Their defining checks are bounded
in the supplied sets.  Ambient ZF proves existence and
uniqueness, and bounded absoluteness identifies their meaning
in these larger ranks; no ZF assumption about
$V_{\eta+\omega}$ is used.  Elementarity and uniqueness
therefore make $J$ carry the source table to the target table.

The usual atomic truth proof, and the decision and witness
dense sets for the compound clauses, give the truth lemma.
Consequently
\[
j(\tau^{G_{\bar\gamma}})
       =J(\tau)^{G_\gamma^*}\qquad(\tau\in N_{\bar\gamma})
\]
is well-defined and elementary.  Transfer forcing witnesses
for both a formula and its negation to obtain agreement in
both directions.  The rank equalities already proved
identify its domain and target with $W_{\bar\gamma},W_\gamma$.
Check-name covariance gives agreement with $J$ on ground
elements, including $e$ and every smaller ordinal.
Replacement on $N_{\bar\gamma}$ forms the graph, whose rank
is below $\gamma+\omega<\Lambda$.

For internal coded elementarity, use the set evaluation graphs
on names and finite assignments to express the truth lemma
as one predicate of the internal formula code in the quotient.
The constructor arguments give definable induction on that
syntax, hence truth for all internal codes.  This avoids
external induction on possibly nonstandard formulas and
completes the stated lift.
\end{proof}

Let $\mathcal D_{\mathrm W}$ be a fixed finite list of formulas
defining this iteration, its completed prefixes and raw limits,
orders and tops, cutoff table, bounded normalized name pools,
projections, empty-tail inclusions, prefix-replacement maps,
the complete codes $a_\eta$, and marked stages.
...
For every $k\geq1$ there is an effectively obtained first-order formula
$\vartheta_k(p,e,\vec\tau)$, independent of the endpoint, with the
following property.  At any marked strongly inaccessible endpoint
$\gamma$ where $V_\gamma$ computes the actual prefixes,
$\vartheta_k$ defines over $V_\gamma$ the forcing relation for
$\mathbb Q_\gamma$ for formulas of L\'evy complexity at most $k$,
with both polarities included.  Its truth lemma holds for every
$\mathbb Q_\gamma$-generic over
$(V_\gamma,\Def(V_\gamma))$.
Every ground supercompact endpoint has the required prefix agreement.
\end{lemma}

\begin{proof}
Use the sparse embeddings to regard the completed prefixes as nested.
At a marked endpoint,
\[
\mathbb Q_\gamma=\bigcup_{\alpha<\gamma}\mathbb Q_\alpha,
\qquad \mathbb Q_\alpha,\leq_\alpha\in V_\gamma,
\]
and the projections are complete.
Every name $\tau\in V_\gamma$ uses a bounded prefix.
Indeed its transitive syntactic closure belongs to $V_\gamma$,
and the least prefix indices of its conditions form a ground function
with bounded range, by strong inaccessibility.
The endpoint rank theorem says that these low names represent exactly
$W_\gamma$.

For a bounded formula and given low names and a condition, compute
ordinary set forcing at any prefix containing those data.
Completeness and bounded absoluteness between transitive extensions
make the answer independent of the prefix.
The uniform bounded-formula forcing predicate, including its
subname and finite evaluation certificates, is first-order over
$V_\gamma$.  The auxiliary sets fit there by the bounded name ranks.

Extend this predicate using the usual negation and conjunction
clauses and the existential density clause
\[
p\Vdash\exists x\,\psi(x,\vec\tau)
\ \longleftrightarrow\
\forall q\leq p\ \exists r\leq q\
\exists\sigma\,
\bigl(\sigma\text{ is a low name}\ \land\
r\Vdash\psi(\sigma,\vec\tau)\bigr).
\]
These are first-order formulas after substituting the fixed iteration
definitions.  Conditions here range over the definable endpoint
forcing; they must be counted as unbounded quantifiers in the final
complexity calculation.
Induction proves the truth lemma.  Existential witnesses have low
names, and the required decision and witness classes are definable
and dense.  Every ambient generic meets them, because a class
definable over the set $V_\gamma$ is externally a ground set.

The resulting low-name structure satisfies $\ZFC$ even for a generic
only over $(V_\gamma,\operatorname{Def}(V_\gamma))$.
To see this, form externally the set name
\[
\dot U=\{(\tau,1_{\mathbb Q_\gamma}):
      \tau\in V_\gamma\text{ is a }\mathbb Q_\gamma\text{-name}\}.
\]
For each fixed formula, its ordinary set-forcing relation with all
quantifiers relativized to $\dot U$ agrees with the preceding
low-name density recursion.  The marked-rank theorem says that the
top forces each fixed standard $\ZFC$ axiom in $\dot U$.
Its corresponding low-name forcing assertion is definable over
$V_\gamma$, so the class-generic truth lemma yields that axiom in
the low-name structure.  No extension of the given filter to an
ambient generic is assumed.

For a fixed L\'evy level $k$, apply the fixed-formula construction to
the one partial satisfaction formula $\operatorname{Sat}_k$.
Use the check name of the formula code and the canonical name of
the finite tuple of parameters.  The preceding $\ZFC$ conclusion
justifies interpreting $\operatorname{Sat}_k$ in the low-name
structure, so this gives one uniform formula $\vartheta_k$ with
the required truth clause for every valid code at that level.
No definition for formulas of arbitrary complexity at once is used.

Finally, a ground supercompact $\gamma$ is $C_2$-correct.
The canonical presentation operations and the positive and negative
restoration tests agree at $V_\gamma$, so uniqueness of the recursion
identifies its internal prefixes with the actual ones.
\end{proof}

{\color{auditgreen}
For definiteness, fix the following unpacked presentation of the formula from Lemma~\ref{lem:Woodin-endpoint-forcing}.  Let $T(\psi)$ be the low-name forcing translation constructed in its proof, with every graph formula from $\mathcal D_{\mathrm W}$ substituted.  Let $\operatorname{Sat}^{\Sigma}_k(m,e,b)$ be the fixed partial truth formula for $\Sigma_k$ codes, with arity $m$, formula code $e$, and assignment $b$.  For $k\geq1$ take
\[
\begin{split}
\vartheta_k(t,p,m,e,b)={}&
\bigl(t=p\ \land\ T(\operatorname{Sat}^{\Sigma}_k)(p,m,e,b)\bigr)\\
&\lor\bigl(t\ne p\ \land\ T(\neg\operatorname{Sat}^{\Sigma}_k)(p,m,e,b)\bigr).
\end{split}
\]
The extra parameter selects the polarity; a tag different from $p$ exists in every endpoint.  In the truth clause, $m,e$ are represented by check names and $b$ by the canonical tuple name.  This is a formula in the membership language, so its dictionary predicates have already been eliminated.  Fix these five free variables without adding a tuple-decoding formula, and put
\[
r_k=c_{\mathcal D_{\mathrm W}}(\{\vartheta_k\})
    =q(\vartheta_k)+3.
\]
The equality follows because negation does not change the quantifier count and every subformula has count at most that of the whole formula.  Thus this bound refers to the displayed forcing formula itself, with both polarities, and uses the positive index $k$ of the required $\Sigma_k$ window.
}
...
\begin{proof}
The endpoints are marked and compute their actual prefixes.
Write $G_\theta$ for the induced prefix generic.
Every parameter in $W_\theta$ has a name in $V_\theta$.
By the choice of $r_k$ and predicate elimination, for each relevant
formula $\varphi$, tuple of low names $\vec\tau$, and
$p\in\mathbb Q_\theta$, the forcing assertions
\[
p\Vdash_{\mathbb Q_\theta}\varphi(\vec\tau)
\quad\text{and}\quad
p\Vdash_{\mathbb Q_\Lambda}\varphi(\vec\tau)
\]
are equivalent, and the same holds for $\neg\varphi$.
If $W_\theta$ satisfies $\varphi(\vec\tau^{G_\theta})$,
the truth lemma supplies a witnessing $p\in G_\theta$.
Transfer its forcing assertion and apply soundness to obtain the
same truth in $W_\Lambda$.
Applying the identical argument to $\neg\varphi$ proves agreement
in both directions, hence the required $\Sigma_k$-elementarity.
\end{proof}
...
\begin{proof}
For $\gamma\leq\Lambda$, write $M_\gamma=(V[G])_\gamma$ for the rank-$\gamma$ part of $M$.  Let $k=N+1$ and put $r=r_k$.  Let $\Xi_N$ be the finite set of formulas asserting that $\theta\in C^{(r)}$, that $\theta$ is supercompact in Woodin's sense, and that $x=a_\theta$ is the actual complete iteration code defined by $\mathcal D_{\mathrm W}$.  Thus the captured code includes the cutoff table and all six forcing tables.  The two complexity bounds needed below are
\[
c_N=c_{\mathcal D_{\mathrm W}}(\Xi_N),\qquad
s_N=\max\{r,d_{\mathrm W},s_{\mathcal D_{\mathrm W}}(\Xi_N)\}.
\]
Let $t_N$ be four more than the maximum of $s_N$ and the bounds $\ell$ for $E_{s_N}(\xi)$, the assertion that the $E_{s_N}$-cardinals are unbounded, membership in $C^{(c_N)}$, and the assertion that $C^{(c_N)}$ is unbounded, after full predicate elimination.  These bounds include both polarities.  Compute in order $r$, $\Xi_N$, $c_N$, the reflected formula $\chi_{\mathcal D_{\mathrm W},\Xi_N}$, $s_N$, and then $t_N$.  Once the coding is fixed, this is a noncircular primitive-recursive construction in $N$.
...
Fix $\eta<\Lambda$ above $\mu$, and choose an $E_{s_N}$-cardinal $\delta$ with $\eta<\delta<\Lambda$.  Let $\delta<\theta<\Lambda$ be another such cardinal and let $\alpha<\theta$.  Choose $\rho\in C^{(c_N)}\cap\Lambda$ above $\theta+\omega$, and put $x=a_\theta$.  The full-code bound gives $x\in V_{\theta+\omega}\subseteq V_\rho$.  Lemma~\ref{lem:ordinary-correct-supercompact} and the specified recursion show that these objects satisfy $\Xi_N$.  Lemma~\ref{lem:finite-reflection} gives
\[
...
\quad J_0(\bar x)=a_\theta.
\]
The reflected instances of $\Xi_N$ give $\bar\theta\in C^{(r)}$, show that $\bar\theta$ is supercompact in Woodin's sense, and identify $\bar x$ with the actual $a_{\bar\theta}$.  This identification uses the finite correctness bound for the full eliminated code predicate.  It is stronger than recovering a source forcing carrier alone.
...
Since $a_{\bar\theta}\in V_{\bar\theta+\omega}$, the restriction retains
$J(a_{\bar\theta})=a_\theta$.  The map $J$ therefore satisfies
Theorem~\ref{thm:Woodin-forcing}(3), with
$\bar\delta<\bar\theta<J(\bar\delta)=\delta<\theta$.
It gives directly an elementary
$e:M_{\bar\theta}\longrightarrow M_\theta$ with critical point
$\bar\delta$, $e(\bar\delta)=\delta$ and $e(\bar\alpha)=\alpha$.
The last equality uses agreement with the ground map on ordinals.
The graph belongs to $M$ by the theorem's bound
$\rank(e)<\theta+\omega<\Lambda$.
The ranks $\bar\theta$, $\theta$, and $\Lambda$ are marked and belong
to $C^{(r)}$.
...
\begin{proof}
Work in $\ZFC$.  Fix a set-sized finitary language $L$, and let $B$
be the disjoint union of $\omega$ and the set of pairs $(\varphi,n)$,
where $\varphi$ is an $L$-formula with free variables among the first
$n$ variables.  Choose a binary relation $T$ on $B$ whose only
endomorphism is the identity \cite[Theorem~2]{VopenkaPultrHedrlin}.
This one set parameter will be common to every coded $L$-structure.

For a structure $A$ with domain $D$, use a fixed finite relational
signature with three unary sorts: labels $B$, elements $D$, and
finite assignments $D^{<\omega}$.  Its other relations are the
binary relation $T$ on labels, a binary length relation between
assignments and natural-number labels, a ternary evaluation
relation $(t,i,a)$ saying $t(i)=a$, and a binary satisfaction
relation between $(\varphi,n)$ and assignments of length $n$.
Use disjoint tags and the canonical satisfaction relation for the
set structure $A$.  This gives a unique code $\operatorname{Enc}(A)$
by a fixed first-order definition from $A,L,T$.

An elementary map between two such codes induces an endomorphism
of $(B,T)$, so it fixes every label.  Its restriction to the element
sort is a map $f$, and the length and evaluation relations force
the image of an assignment $t$ to be $f\circ t$.  Preservation and
reflection of the satisfaction relation therefore give
\[
A\models\varphi[t]\quad\Longleftrightarrow\quad
A'\models\varphi[f\circ t]
\]
for every $L$-formula.  Thus $f:A\to A'$ is elementary.
The code determines $A$ from its element sort and its satisfaction
relation on atomic formulas.

For completeness, this fixed finite relational signature can be
encoded in one binary relation $E$ and one unary relation $R$.
Use finitely many marker vertices forming a directed path, with
$R$ true exactly on them.  Take a domain marker, a marker for each
relation symbol, and a marker for each argument position.  For
each true relation tuple add a record vertex and its position
vertices.  Put edges from the domain marker to the original
elements, from the relation marker to the record, from the record
to its position vertices, from each position marker to the
corresponding position vertex, and from that position vertex to
the indicated element.  Apart from the marker path these are
all the edges.  Every marker is definable as a specified finite
successor of the first $R$-vertex.  The original domain is
therefore definable, and a relation is defined by existence of
the appropriate record and its marked position vertices.
Relativizing quantifiers to that domain proves, by induction on
formulas, that an elementary map of these graph codes restricts
to an elementary map of the finite relational codes.

Let $\operatorname{Code}$ be this composite canonical coding.
It has a definable inverse on its image.  For a class defined by
$\Phi(A,z,L)$, define its coded class by
\[
\exists A\bigl(\Phi(A,z,L)\land
                       X=\operatorname{Code}(A,L,T)\bigr).
\]
If the class of codes were a set, Replacement using the decoding
would make the original class a set.  Thus properness is
preserved, and distinct codes decode to distinct structures.
The fixed coding definition has finite L\'evy complexity,
independently of the size of $L$ and the values of $z,T$.
Substitute this definition, calculate a syntactic bound for
each of the finitely many input formulas, and take $N\geq1$
large enough to put them all in $\Pi_{N+1}$ form.
Parameterized $\VP(\Pi_{N+1})$ for the fixed graph signature
now gives every required instance.  The construction of this
bound is effective.
\end{proof}
\endgroup
...
Choose a standard $N\geq1$ for the finitely many instances of $\VP$
in $F$, as in Lemma~\ref{lem:finite-language-reduction}.
Choose $t_N$ as in Theorem~\ref{thm:finite-restoration} and choose
in $\mathfrak M$ a sufficiently large $E_{t_N}$-cardinal $\Lambda$.
The finite computation of $t_N$ at standard $N$ is correct inside
$\mathfrak M$, even if its natural numbers are nonstandard.

The countable model $\mathfrak M$ need not be externally well-founded.
Enumerate externally all of its internal dense subsets of
$\mathbb Q_\Lambda$ and construct a descending sequence meeting them.
Its upward closure is an $\mathfrak M$-generic filter $H$.
Define $\mathfrak M[H]$ as the quotient of its internal names, where
\[
\tau\sim_H\sigma\quad\Longleftrightarrow\quad
\exists p\in H\ \mathfrak M\models p\Vdash\tau=\sigma,
\]
and define membership by the analogous forced-membership clause.
Atomic forcing identities make the quotient well-defined.
Induction on each external standard formula gives its truth lemma;
the decision and witness dense sets are internal sets by Separation.
No evaluation of names by external well-founded recursion is used.

Let $u$ be the internal set $V_{\check\Lambda}$ of
$\mathfrak M[H]$, and let $W$ be its external membership structure.
Truth of a standard formula in $W$ is truth of its relativization
to $u$ in the quotient.  Read Theorem~\ref{thm:finite-restoration}
as the corresponding set-forcing assertions, one standard axiom
or sentence at a time.  The truth lemma gives $W\models\ZFC$
and unboundedly many $C^{(n)}$-extendible cardinals for
$1\leq n\leq N$.

For a parameterized $\Pi_{N+1}$ class in $W$, add a vacuous
existential quantifier to view its definition as $\Sigma_{N+2}$.
Choose a $C^{(N)}$-extendible $\kappa$ with the type and all
parameters in $H_\kappa^W$.  If the class is proper, choose a
member outside $H_\kappa^W$.  Bagaria's Theorem~4.11
\cite{BagariaCn} supplies an elementarily embeddable member inside
$H_\kappa^W$, so the two members are distinct.
Thus $W$ satisfies the required parameterized
$\VP(\Pi_{N+1})$ scheme.  Lemma~\ref{lem:finite-language-reduction}
gives every instance of $\VP$ in $F$, and therefore $W\models F$.
...
\begin{proof}
{\color{auditgreen}
The model in \eqref{eq:full-Solovay} is the full Solovay model in the precise convention of \cite[Definition~4 and the following paragraph]{SakaiTanno}.  Since a measurable cardinal is inaccessible in $\ZFC$, Solovay's argument applies.  For the parameter convention, use the intermediate ground-and-real-definable sets in \cite[III, Sections~1.3--1.11]{Solovay}: those arguments explicitly allow an arbitrary ground-set parameter.  Absorb the real parameter into a bounded collapse extension.  Over that ground, the random-real Boolean value gives a Borel approximation to each such set, and its error is contained in the Borel null set of non-random reals.  The Cohen/category version gives the Baire property.  For an uncountable such set, the construction from pairs of mutually generic collapsing maps gives a continuous injection of $2^\omega$ into it.  The Borel and perfect-set codes are reals and therefore belong to $\mathcal S\!ol$; their interpretations agree with those in $V[G]$.

For the hereditary passage, the full model is closed under countable sequences taken in $V[G]$.  Indeed, choose in $V[G]$ defining data for the terms of a sequence.  The reals in these data combine into one real.  The countable sequences of ground parameters and of ordinal parameters have bounded-stage names: the $\kappa$-chain condition bounds the union of their deciding antichains.  Such a bounded-stage name and a real coding its generic filter define the whole sequence from allowed parameters.  Heredity follows from heredity of its terms.  This also transfers a countable enumeration when the set of reals is countable in $V[G]$.  A dependent-choice sequence chosen in the ambient $\ZFC$ extension consequently belongs to $\mathcal S\!ol$.  Together with the usual generalized-HOD proof of $\ZF$, these facts give $\mathcal S\!ol\models\ZF+\DC+\LM+\BP+\PSP$, as in \cite[III, Sections~2.4--2.11]{Solovay}.
}
...
Consequently, by \cite[Proposition~9.4]{KaragilaSchilhan}, the symmetric extension arising from
...
{\color{auditgreen}
In the reverse inclusion in the proof of \cite[Proposition~9.4]{KaragilaSchilhan}, the comparison generics must generate the same extension, as required by its Lemma~9.3.  This restriction is definable from ground-set parameters.  Fix in $V$ the set $T$ of canonical Boolean names for subsets of $\check{\mathbb B}$, and consider only generics $H'$ in $V[H]$ for which every subset of $\mathbb B$ in $V[H]$ is $\tau^{H'}$ for some $\tau\in T$.  The actual $H$ has this property.  Conversely, such an $H'$ has $H\in V[H']$, so $V[H']=V[H]$.  The comparison argument therefore applies without using $H$ as a defining parameter.
}
\end{proof}

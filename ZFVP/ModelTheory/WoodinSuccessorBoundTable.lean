...
noncomputable def woodinSuccessorBoundTable (k s K B i I : V) : V :=
  ...
  forcingFamilyNext (succ k) B
    (forcingSuccessorBound (twoStepConditions P R Q ∅) ((forcingCodeP s) ‘ i)
      (twoStepProjection P R Q ∅) (B ‘ k) I)

theorem IsWoodinIteration.successor_bound_table {δ k s K B i I : V} [IsOrdinal k]
    ...
    IsCoherentForcingBound (succ (succ k)) (forcingCodeP (woodinIterationSuccessor k s K))
      (forcingCodeR (woodinIterationSuccessor k s K)) (forcingCodeπ (woodinIterationSuccessor k s K))
      (woodinSuccessorBoundTable k s K B i I) i I := by
  ...
  have he := hB.extend hcB hi
  simpa only [woodinIterationSuccessor, forcingSuccessorCode, forcingIterationCodeNext,
    forcingCodeP_code, forcingCodeR_code, forcingCodeπ_code, woodinSuccessorBoundTable] using he

...
theorem internalRank_formula_truth_of_inaccessible (hδ : IsChoicelessInaccessible δ)
    ...
    φ.Evalb (show Fin n → SetDomain (hierarchy (A.check δ)) from fun i ↦
      (⟨A.ofName (v i), (A.mem_checked_hierarchy_iff_low_name_of_inaccessible hδ (hP ▸ P.property) _).mpr
  ...
  have he' : (forcingFormula P R φ (standardTuple w)).val =
    ...
    convert he using 1
    rfl
  exact (A.rankName_formula_truth_of_inaccessible hδ (hP ▸ P.property) φ v hv).trans

...
/-! The inverse-limit step of the canonical quotient-bound induction, the analogue at a
packed limit coordinate of `woodinQuotientBoundAt_successor_normalized`. -/
...
theorem woodinQuotientBoundAt_inverse_normalized_of_raw [Countable V] {δ θ i j p α : V}
  ...
  have hdata (G : Set V) (hG : IsExternalForcingGeneric
    ...
    · intro a ha
      ...
      rw [hPi, hRi] at hL
      exact A.projectionQuotient_separative_of_lift hsplit.projection.maps hL hqQ hwQ hdiG
        (fun r hr hrq b hb hbr hbd ↦
          hraw (kpair.π₁ (d ‘ j)) hw r hr hrq b hb hbr (hwi ▸ hbd))
    · intro β hβ hβα

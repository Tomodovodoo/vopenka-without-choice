...
theorem woodinNamedCollapse_restoration_forcing_iff {P R one p : V}
    (hR : IsForcingPreorder P R) (htop : IsForcingTop P R one) (hp : p ∈ P)
    (Q κ δ : ForcingName P)
    (hQ : p ∈ forcingFormula P R totalWoodinCollapseFormula (standardTuple ![Q.val, κ.val, δ.val]))
  ...
  have hφ : p ∈ forcingFormula P R φ (standardTuple ![Q.val, S.val, t.val, κ.val, δ.val]) := by
    ...
    exact ⟨hQ,
      reverseInclusionOrderName_forces hR htop hp Q, emptyName_forces hR htop hp, hδ⟩

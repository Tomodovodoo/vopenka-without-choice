...
theorem eventually_hartogsRestoration_forcing_eq_countable [Countable V]
  ...
  let v : Fin 2 → ForcingName P :=
    ![⟨hartogsNumberName P R (checkName one κ), hartogsNumberName_isName _ _ _⟩, ⟨checkName one γ, checkName_isName ht.1 γ⟩]
  ...
  have hk : A.ofName (v 0) = hartogsNumber (A.check κ) := A.hartogsName_value τ
  have hl := hall G hG (A.check ξ) ((A.check_mem_iff _ _).mpr hβξ) hi (w 0) (w 1) hk rfl
  ...
  have hl' : IsWoodinLocalRestoration (w 0) (w 1) ↔
      IsWoodinLocalRestoration (A.ofName (v 0)) (A.ofName (v 1)) := by
    rw [hk]
    exact hl
  ...
  have hn : (fun i ↦ (v i).val) = ![hartogsNumberName P R (checkName one κ), checkName one γ] := by
    funext i

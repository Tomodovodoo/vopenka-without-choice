...
/-! # The successor step with all definable directed sets at once

`ZFVP.exists_stage_successor` takes `ω`-indexed enumerations of the inseparable pairs and of the
definable directed sets with no last element. Enayat's construction feeds the successor step
*every* definable directed set with no last element of the current stage, and the pairs it has to
keep inseparable are given as sets of points of the carrier, not as predicates on the stage.

`ZFVP.exists_extension_all_directed` already packages the first half: Lemma A.2 with the
enumeration of all the definable directed sets built in. This file glues that to the transport onto
the carrier (`ZFVP.exists_carrier_extension`), giving the form of the successor step the
`ω₁`-length construction uses.

The inseparable pairs are handed over as pairs of subsets of the carrier `Ω`, both of them inside
`S.carrier`. Read inside the stage `S` such a pair is the pair of predicates
`fun x ↦ (x : Ω) ∈ p.1`, `fun x ↦ (x : Ω) ∈ p.2`, and read inside the larger stage `T` it is the
same two predicates on the larger carrier. Since the two subsets lie inside `S.carrier`, the
predicates on `↥T.carrier` are exactly the images along the inclusion of the predicates on
`↥S.carrier`, which is the shape Lemma A.2 preserves; `ZFVP.Inseparable.congr` turns one shape into
the other.
-/
...
/-- The successor step of Enayat's `ω₁`-length construction, in the form the construction uses it.

Given a countable stage `S` on a carrier `Ω` of size `ℵ₁` and a countable set `O` of pairs of
subsets of `S.carrier`, there is a countable stage `T` on a larger subset of `Ω` such that:
the two membership relations agree on `S.carrier`; the inclusion of `S` into `T` is elementary;
every pair of `O` that is inseparable in `S` is still inseparable in `T`, read as the same two
subsets of `Ω`; and every definable directed set with no last element of `S` has an element of `T`
above all of its old elements.

This is `ZFVP.exists_extension_all_directed` followed by `ZFVP.exists_carrier_extension`. -/
theorem exists_stage_successor_all {Ω : Type u} (hΩ : Cardinal.mk Ω = Cardinal.aleph 1)
  ...
  obtain ⟨N, _, _, _, j, hins, hbound⟩ :=
    exists_extension_all_directed (M := ↥S.carrier) (F '' O₀) (hO₀.image F)
  ...
  have key : ∀ P : Set Ω, P ⊆ S.carrier → ∀ y : ↥T.carrier,
    ...
    · rintro ⟨a, ha, hya⟩
      ...
      rw [hy]
      exact ha
  refine ⟨T, hAB, hBcount, ?_, ⟨(StageModel.ofEquivSymmMap B e).comp j, fun x ↦ ?_⟩, ?_, ?_⟩
  · intro x y

import ZFVP.ModelTheory.UsubaLSStageDC
import ZFVP.ModelTheory.UsubaLSEnumerationBounds
...
theorem usubaStage_successor_enumeration_of_ls [Countable V]
...
theorem usubaStageSetsHaveDCEnumerations_of_ls [Countable V]
  ...
  · have hDC : ∀ β ∈ A.check k, InternalDependentChoiceAt β :=
      usubaStageDCBelow_semantics (usubaStageDCBelow_all_of_ls hLS k)
        ((T).stageFilter_generic hG k)
    have hX : rank X ⊆ A.check k := by
      rw [hrank k hik]
      exact (A.checkEmbedding.subset_iff _ _).mpr (subset_union_right i ρ)
    have htarget := A.usuba_rank_target_bound_of_ls hLS hAC k hDC hX
    obtain ⟨γ, hγ, hdcγ, e, he, her⟩ :=
      usubaStage_successor_enumeration_of_ls hLS hG k hAC htarget
    let := hγ
...
theorem usubaSubsetsStabilize_of_ls [Countable V]
    (hLS : ∀ α : V, IsOrdinal α → ∃ κ : V, α ∈ κ ∧ IsLSCardinal κ)
    {G : Set V} (hG : IsGenericForDefinableDenseClasses (T).Condition (T).LE G) :
    (T).SubsetsStabilize hG := by
  apply (T).subsetsStabilize_of_enumerations hG (usubaStageSetsHaveDCEnumerations_of_ls hLS hG)

import ZFVP.ModelTheory.InfinitaryCountablyDirectedWeakStructure
...
/-- Countable boundedness gathers all conjunct witnesses into a common stage.
The parameters then agree there by the quotient's equality criterion. -/
theorem stageEval_conj {n} (f : ℕ → Formula L n)
    (hfi : ∀ m, ⟨n, f m⟩ ∈ FragmentClosure.carrier S) (b : Fin n → C.Limit) :
    C.StageEval (.conj f) b ↔ ∀ m, C.StageEval (f m) b := by
  classical
  constructor
  · rintro ⟨i, a, ha, ht⟩ m
    exact ⟨i, a, ha, ht m⟩
  · intro h
    choose j a ha ht using h
    obtain ⟨k, hk⟩ := C.bounded j
    obtain ⟨i, b', hb'⟩ := C.tuple_cover b
    let l := C.upper i k
    have hi : i ≤ l := C.le_upper_left i k
    have hj (m : ℕ) : j m ≤ l := (hk m).trans (C.le_upper_right i k)
    refine ⟨l, C.map hi ∘ b', ?_, ?_⟩
    · rw [← Function.comp_assoc, C.fromStage_comp, hb']
    · intro m
      have hm := (C.map (hj m)).elementary (f m) (hfi m) (a m) |>.mpr (ht m)
      have he : C.map (hj m) ∘ a m = C.map hi ∘ b' := by
        funext t
        apply (C.fromStage_eq_iff (a m t) (b' t) (hj m) hi).mp
        exact congrFun ((ha m).trans hb'.symm) t
      rwa [he] at hm
...
theorem stageEval_exs {n} {φ : Formula L (n + 1)}
  ...
  · rintro ⟨x, hx⟩
    ...
    let k := C.upper i j
    have hi : i ≤ k := C.le_upper_left _ _
    have hj : j ≤ k := C.le_upper_right _ _
...
theorem limit_weak_truth {n} (φ : Formula L n)
  ...
  | conj f ih =>
    ...
    exact (forall_congr' fun l ↦ ih l (hc l) b).trans (C.stageEval_conj f hc b).symm
  | exs φ ih =>
    have hc := subformulas_closed hφ (Or.inr (Formula.self_mem_subformulas φ))
...
noncomputable def limitModel : WeakLimitStructure.{u,max v w} L where
  Domain := C.Limit
...
noncomputable def limitEmbedding (i : I) :
    WeakLimitEmbedding (FragmentClosure.carrier S) (C.model i) C.limitModel where
  toFun := C.fromStage i
...
hypothesis is required. The index set need not be countable. -/
theorem exists_weak_limit :
    ∃ N : WeakLimitStructure.{u,max v w} L,
      ∃ e : ∀ i, WeakLimitEmbedding (FragmentClosure.carrier S) (C.model i) N,

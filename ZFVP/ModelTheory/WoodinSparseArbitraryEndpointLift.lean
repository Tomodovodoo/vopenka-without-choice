...
theorem ForcingContext.woodinSparseEndpoint_exists_restricted_internalLift
    ...
    ∃ g ∈ hierarchy (A.check Ω),
      IsCodedMembershipEmbedding (hierarchy (A.check γ)) (hierarchy (A.check δ)) g := by
  obtain ⟨G, hG, rfl⟩ := woodinSparseEndpoint_raw_generic_presentation hΩ hAC A hAP hAR hone
  apply woodinSparseFixedPointContext_exists_restricted_internalLift
    hΩ hAC hG hγ hδ hγδ hfixγ hfixδ he hP
  intro p hp
  ...
  have hp' : p ∈ forcingProjectionGeneric Pγ Rγ πγ
      (woodinSparseGenericContext hΩ hAC (subset_refl Ω) hG).G := by
    rwa [hprojγ]
  have hh := hg p hp'
  rwa [hprojδ] at hh

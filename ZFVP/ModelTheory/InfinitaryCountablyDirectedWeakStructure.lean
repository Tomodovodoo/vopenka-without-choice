import ZFVP.ModelTheory.InfinitaryCountablyDirectedWeakChain
...
theorem stage_func_congr {n i j} (f : L.Func n)
  ...
  apply (C.fromStage_eq_iff _ _ (C.le_upper_left i j) (C.le_upper_right i j)).mpr
  rw [(C.map (C.le_upper_left i j)).func, (C.map (C.le_upper_right i j)).func]
  congr 1
  funext l
  exact (C.fromStage_eq_iff (b l) (c l) (C.le_upper_left i j)
    (C.le_upper_right i j)).mp (congrFun he l)

noncomputable def tupleStage {n} (b : Fin n → C.Limit) : I := (C.tuple_cover b).choose

import ZFVP.ModelTheory.RankWoodinSuccessorAt
...
def rankWoodinSuccessorAtConclusionFormula : SetTheorySemisentence 7 :=
  let φ := boundedDomainParametersFormula woodinSuccessorAtFormula
  let ψ := woodinSuccessorAtFormula.subst (fun i ↦ .bvar i.succ)
  φ 🡘 ψ

def rankWoodinSuccessorAtAgreementFormula : SetTheorySemisentence 8 :=
  f“U ξ P R o κ γ z. U = !hierarchyFormula ξ ∧ !choicelessInaccessibleFormula ξ ∧
    ...
    !forcingPreorderFormula P R ∧ !forcingTopFormula P R o ∧ !IsOrdinal.dfn γ →
      !rankWoodinSuccessorAtConclusionFormula U z P R o κ γ”

variable {V : Type*} [SetStructure V] [Nonempty V] [V↓[ℒₛₑₜ] ⊧* 𝗭𝗙]

instance rankWoodinSuccessorAtConclusionFormula_defined :
    Defined (fun v : Fin 7 → V ↦
      (boundedDomainParametersFormula woodinSuccessorAtFormula).Evalb v ↔
        v 1 = woodinSuccessorAt (v 2) (v 3) (v 4) (v 5) (v 6))
      rankWoodinSuccessorAtConclusionFormula :=
  ⟨fun v ↦ by simp [rankWoodinSuccessorAtConclusionFormula, Semiformula.eval_substs]⟩

private theorem forall_seven_eq_successor {W : Type*} (a b c d e f g : W)
...
private theorem forall_three_eq_successor {W : Type*} (a b c : W) (F : W → W → W → Prop) :
    (∀ u v w, u = a → v = b → w = c → F u v w) ↔ F a b c :=
  ⟨fun h ↦ h a b c rfl rfl rfl, fun h u v w hu hv hw ↦ by subst u v w; exact h⟩

theorem eval_rankWoodinSuccessorAtAgreementFormula (v : Fin 8 → V) :
    rankWoodinSuccessorAtAgreementFormula.Evalb v ↔
      (v 0 = hierarchy (v 1) → IsChoicelessInaccessible (v 1) →
        ...
        ((boundedDomainParametersFormula woodinSuccessorAtFormula).Evalb ![v 0, v 7, v 2, v 3, v 4, v 5, v 6] ↔
          v 7 = woodinSuccessorAt (v 2) (v 3) (v 4) (v 5) (v 6))) := by
  simp [rankWoodinSuccessorAtAgreementFormula, Semiformula.eval_nestFormulae,
    Matrix.vecForall_iff, Fin.forall_fin_succ, forall_seven_eq_successor, forall_three_eq_successor]

theorem rankWoodinSuccessorAtAgreement_countable [Countable V] (v : Fin 8 → V) :
    rankWoodinSuccessorAtAgreementFormula.Evalb v := by
  apply (eval_rankWoodinSuccessorAtAgreementFormula v).mpr
  ...
  have he := rank_woodinSuccessorAt_val_countable hξ P R o k g
  ...
  have hb := eval_boundedDomainParametersFormula woodinSuccessorAtFormula (hierarchy (v 1)) ![z, P, R, o, k, g]
  ...
  rw [hv] at hb
  have heq : z = woodinSuccessorAt P R o k g ↔ v 7 = woodinSuccessorAt (v 2) (v 3) (v 4) (v 5) (v 6) := by
    constructor
    · intro h
      exact (congrArg Subtype.val h).trans he
    · intro h
      exact Subtype.ext (h.trans he.symm)
  exact hb.trans ((Defined.eval_iff (φ := woodinSuccessorAtFormula) ![z, P, R, o, k, g]).trans heq)

theorem rank_woodinSuccessorAt_val {ξ : V} [IsOrdinal ξ]
  ...
  let z := woodinSuccessorAt P R one κ γ
  have hv := eval_of_countable_zf rankWoodinSuccessorAtAgreementFormula
    (fun _ _ _ _ _ v ↦ rankWoodinSuccessorAtAgreement_countable v)
    ![hierarchy ξ, ξ, P.val, R.val, one.val, κ.val, γ.val, z.val]
  have he := (eval_rankWoodinSuccessorAtAgreementFormula _).mp hv rfl hξ P.property R.property
  ...
  apply he.mp
  have hb := eval_boundedDomainParametersFormula woodinSuccessorAtFormula (hierarchy ξ) ![z, P, R, one, κ, γ]
  have hn : (fun i : Fin 6 ↦ ((![z, P, R, one, κ, γ] : Fin 6 → SetDomain (hierarchy ξ)) i).val) =
    ...
    exact Fin.cases rfl (fun j ↦ Fin.cases rfl (fun k ↦ Fin.cases rfl
      (fun l ↦ Fin.cases rfl (fun m ↦ Fin.cases rfl (fun n ↦ Fin.cases rfl
        (fun t ↦ Fin.elim0 t) n) m) l) k) j) i
  rw [hn] at hb
  exact hb.mpr ((Defined.eval_iff (φ := woodinSuccessorAtFormula) ![z, P, R, one, κ, γ]).mpr rfl)

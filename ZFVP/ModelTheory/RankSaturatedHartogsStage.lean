...
theorem rank_saturatedHartogsStageAt_val_countable (hξ : IsChoicelessInaccessible ξ)
  ...
  let Q := saturatedHartogsPosetName P R one κ γ
  ...
  have hQval := rank_saturatedHartogsPosetName_val_countable hξ P R one κ γ hR ht hγ
  ...
  have hC : (twoStepConditions P R Q (∅ : SetDomain (hierarchy ξ))).val =
      twoStepConditions P.val R.val (saturatedHartogsPosetName P.val R.val one.val κ.val γ.val) ∅ := by
  ...
  have hT : (twoStepOrder P R Q S (∅ : SetDomain (hierarchy ξ))).val =
      twoStepOrder P.val R.val (saturatedHartogsPosetName P.val R.val one.val κ.val γ.val)
        (saturatedHartogsOrderName P.val R.val one.val κ.val γ.val) ∅ := by
  ...
  unfold woodinStageCode saturatedHartogsStageAt

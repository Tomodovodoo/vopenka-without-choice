...
/-! Proto rank-Berkeley embeddings give an internal ZF model of every positive UE instance. -/
...
theorem IsProtoRankBerkeley.exists_zf_unboundedExtendibility {ζ δ : V} (hδ : IsProtoRankBerkeley ζ δ) :
    ∃ Λ : V, IsOrdinal Λ ∧ Λ ⊆ δ ∧ ζ ∈ Λ ∧ internalCofinality Λ = (ω : V) ∧
      IsInternalZFModel (hierarchy Λ) ∧
      ∀ m : ℕ, SetSentenceTrue (unboundedExtendibilitySentence (m + 1)) (hierarchy Λ) := by
  let := hδ.1.1

...
def sigmaOneInternalForcingFormula : SetTheorySemisentence 7 :=
...
theorem sigmaOneInternalForcingFormula_sigmaOne : IsSigmaFormula 1 sigmaOneInternalForcingFormula := by
  unfold sigmaOneInternalForcingFormula
        ...
        (.exs (.and (sigmaOneMembershipFamilyFormula_sigmaOne.subst _)
          (.exs (.and (.bounded (boundedAtomicTruthTableFormula_bounded.subst _))
            (.exs (.and (.bounded (boundedInternalForcingTableFormula_bounded.subst _))
              (.bounded ((CodeExpression.kpair (.var 0) (.var 1)).forcingLookupFormula_bounded.subst _)))))))))))))

variable {V : Type*} [SetStructure V] [Nonempty V] [V↓[ℒₛₑₜ] ⊧* 𝗭𝗙]

theorem eval_sigmaOneInternalForcingFormula {P R D n φ b p : V}
    (hφ : IsMembershipFormulaCode n φ) (hb : b ∈ D ^ n) (hp : p ∈ P) :
    sigmaOneInternalForcingFormula.Evalb ![P, R, D, n, φ, b, p] ↔ InternalForces P R D n φ b p := by
  have hm : sigmaOneInternalForcingFormula.Evalb ![P, R, D, n, φ, b, p] ↔
    ...
    simp [sigmaOneInternalForcingFormula, Matrix.comp_vecCons', Function.comp_def, Matrix.constant_eq_singleton]
  rw [hm]
  have hlocal (U : V) (hU : IsSequenceSupport U) (hP : P ∈ U) (hD : D ∈ U) :
    ...
    · rintro ⟨H, hH, T, hT, ht⟩
      ...
      have hl := (CodeExpression.eval_forcingLookupFormula_two (.kpair (.var 0) (.var 1)) T hbu (hPs p hp) hnu hφu).mp ht
      exact (hT.lookup hφ hb hp).mp hl
    · intro ht
      ...
      apply (CodeExpression.eval_forcingLookupFormula_two (.kpair (.var 0) (.var 1)) _ hbu (hPs p hp) hnu hφu).mpr
      exact (internalForcingTruthTable_lookup hφ hb hp).mpr ht
  constructor
